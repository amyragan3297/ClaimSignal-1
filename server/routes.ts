...
Base your advice on insurance industry best practices, policy interpretation, and negotiation tactics.`
  ...
  // Behavior Prediction endpoint - predict adjuster behavior based on historical patterns
  app.post("/api/predict-behavior", authMiddleware, async (req, res) => {
    try {
      const { adjusterId, claimId } = req.body;
      ...
      // Gather all available data for prediction
      const intelligence = await storage.getAdjusterIntelligence(adjusterId);
      const interactions = await storage.getInteractionsByAdjuster(adjusterId);
      const claims = await storage.getClaimsByAdjuster(adjusterId);
      
      // Build context for AI
      let context = `Adjuster: ${adjuster.name} (${adjuster.carrier})`;
      if (adjuster.region) context += `\nRegion: ${adjuster.region}`;
      ...
      if (intelligence) {
        context += `\n\nPerformance Metrics:`;
        context += `\n- Total Claims: ${intelligence.totalClaims}`;
        context += `\n- Total Interactions: ${intelligence.totalInteractions}`;
        context += `\n- Risk Score: ${intelligence.riskScore}/100`;
        if (intelligence.responsivenessScore) context += `\n- Responsiveness Score: ${intelligence.responsivenessScore}/100`;
        if (intelligence.cooperationLevel) context += `\n- Cooperation Level: ${intelligence.cooperationLevel}`;
        if (intelligence.supplementApprovalRate !== null) context += `\n- Supplement Approval Rate: ${intelligence.supplementApprovalRate}%`;
        if (intelligence.avgDaysToResolution) context += `\n- Avg Days to Resolution: ${intelligence.avgDaysToResolution}`;
        context += `\n- Claims Resolved: ${intelligence.outcomesResolved}, Stalled: ${intelligence.outcomesStalled}, Open: ${intelligence.outcomesOpen}`;
        if (intelligence.patternTags.length > 0) context += `\n- Behavior Patterns: ${intelligence.patternTags.join(', ')}`;
      }
      ...
      if (interactions.length > 0) {
        ...
        context += `\n\nRecent Interactions:`;
        interactions.slice(-10).forEach(i => {
          context += `\n- [${i.date}] ${i.type}: ${i.description?.substring(0, 100) || 'No details'}${i.outcome ? ` → ${i.outcome}` : ''}`;
        });
      }
      ...
      if (claims.length > 0) {
        ...
        context += `\n\nClaim Outcomes: ${Object.entries(statusCount).map(([s, c]) => `${s}: ${c}`).join(', ')}`;
      }
      
      // If specific claim context provided
      let claimContext = "";
      if (claimId) {
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: `You are an expert insurance claims analyst specializing in predicting adjuster behavior patterns. Based on historical data, provide behavioral predictions for this adjuster.

Return a valid JSON object with this structure:
{
  "overallBehavior": "A 2-3 sentence summary of this adjuster's typical behavior pattern",
  "likelyActions": ["Array of 3-5 behaviors this adjuster is likely to exhibit"],
  "warningSignals": ["Array of 2-4 red flags or challenges to watch for"],
  "successIndicators": ["Array of 2-3 positive signs that indicate progress"],
  "negotiationStyle": "aggressive" or "cooperative" or "evasive" or "methodical" or "unpredictable",
  "responseTimeExpectation": "fast" or "average" or "slow",
  "supplementLikelihood": "likely_approve" or "likely_deny" or "will_negotiate" or "insufficient_data",
  "recommendedApproach": "A 2-3 sentence recommendation for working with this adjuster",
  "confidenceLevel": "high" or "medium" or "low" (based on amount of historical data available)
}

Base predictions on actual behavioral patterns from the data provided.`
          },
          {
            role: "user",
            content: `Predict behavior for this adjuster:\n\n${context}${claimContext}`
          }
        ],
        response_format: { type: "json_object" },
        max_tokens: 1024,
      ...
      try {
        ...
        console.error("Error parsing prediction response:", content);
        prediction = {
          overallBehavior: "Unable to generate prediction",
          likelyActions: [],
          warningSignals: [],
          successIndicators: [],
          negotiationStyle: "unpredictable",
          responseTimeExpectation: "average",
          supplementLikelihood: "insufficient_data",
          recommendedApproach: "Gather more interaction data to improve predictions",
          confidenceLevel: "low"
        };
      }

      // Ensure all required fields exist
      prediction.likelyActions = prediction.likelyActions || [];
      prediction.warningSignals = prediction.warningSignals || [];
      prediction.successIndicators = prediction.successIndicators || [];
      prediction.confidenceLevel = prediction.confidenceLevel || "low";
      ...
      res.json({ 
        ...
        dataPoints: {
          ...
          hasRiskAssessment: !!adjuster.riskImpression
        }
      });
    } catch (error) {
